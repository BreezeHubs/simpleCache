...
// NewLruCache 实例化
func NewLruCache(opts ...option) *Cache {
	c := &Cache{
		...
		lruGarbageCollectionRate: 0.8,                             //默认达到80%占用比率，触发lru回收
	}
...
func (c *Cache) Set(key string, value any, expire time.Duration) bool {
	...
	data.element = c.list.PushFront(key)
	c.add(key, data)
...
func (c *Cache) Get(key string) (any, bool) {
	...
	if ok {
		...
		if data.expire == 0 || time.Now().Before(data.createdAt.Add(data.expire)) {
			c.list.MoveToFront(data.element)

			//for e := c.list.Front(); e != nil; e = e.Next() {
			//	fmt.Println("node: ", e.Value) //输出list的值,061234
			//}
...
func (c *Cache) gcLru() {
	...
	rate := float64(c.currentMemorySize) / float64(c.maxMemorySize)
	//fmt.Println("c: ", c.currentMemorySize, c.maxMemorySize)
	//log.Println("r: ", rate, c.lruGarbageCollectionRate, rate > c.lruGarbageCollectionRate)
	if rate > c.lruGarbageCollectionRate {
		ele := c.list.Back()
		if ele != nil {
			if key, ok := ele.Value.(string); ok {
				c.del(key)
			}
		}
	}
